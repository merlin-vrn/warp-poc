...
void max_heapify(heapqueue_t * queue, const HEAPQUEUE_COUNT_TYPE i) {
    ...
    HEAPQUEUE_COUNT_TYPE largest = i;
    if (left <= queue->n && queue->a[left].p > queue->a[largest].p) largest = left;
    if (right <= queue->n && queue->a[right].p > queue->a[largest].p) largest = right;
    ...
    if (largest != i) {
        ...
        queue->a[i] = queue->a[largest];
        queue->a[largest] = temp;
